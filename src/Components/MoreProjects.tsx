...
const MoreProjects = () => {
  ...
  // Render details for the selected project.
  // This version groups consecutive list items and handles inline marks.
  const renderProjectDetails = () => {
    if (!selectedProject)
      return <Typography variant="h6">Select a project</Typography>;

    // Group the blocks so that consecutive list items are in one group.
    ...
    return (
      <Box p={2}>
        ...
        {groupedBlocks.map((block: any, index: number) => {
          ...
          // 2) Render a normal text block.
          if (block._type === 'block') {
            return renderBlock(block, index);
          }
          // 3) Render an image block.
